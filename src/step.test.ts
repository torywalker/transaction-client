...
describe("#constructor()", () => {
  it("should error when name not present", () => {
    expect(
      () => new (Step as any)(undefined, successFn, successFn)
    ).toThrowError("Invalid value provided for name while initalizing class");
  });

  it("should error when fnToExecute not present", () => {
    expect(
      () => new (Step as any)("testName", undefined, successFn)
    ).toThrowError(
